...
pub fn config_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("EXA_CONFIG_DIR").filter(|v| !v.is_empty()) {
        return PathBuf::from(dir);
    }

    #[cfg(windows)]
    let dir = std::env::var_os("APPDATA").map(PathBuf::from).unwrap_or_default().join("exa-cli");

    #[cfg(not(windows))]
    let dir = {
        let home = std::env::var_os("HOME").map(PathBuf::from).unwrap_or_default();
        if cfg!(target_os = "macos") {
            home.join("Library").join("Application Support").join("exa-cli")
        } else {
...
pub fn load() -> Result<Config> {
    ...
    serde_norway::from_str::<Option<Config>>(&text).map(Option::unwrap_or_default).map_err(|e| {
        Error::other(format!("{} is not valid.", path.display()))
            .detail(e.to_string())
            .fix(format!("Fix or delete {}, then run: exa accounts add <name>", path.display()))
    })
}
        ...
        match file.try_lock() {
            ...
            Err(fs::TryLockError::WouldBlock) => {
                return Err(Error::other(format!(
                    "Timed out waiting for the lock at {}. Another exa process may be stuck.",
                    path.display()
                )));
            }
...
mod tests {
    ...
    fn config_serialization_roundtrip() {
        ...
        c.accounts.insert(
            "work".into(),
            AccountConfig { identity: "test-key".into(), added_at: "2026-01-01T00:00:00Z".into() },
        );
        ...
        assert_eq!(name, "work");
        assert_eq!(acct.identity, "test-key");
    }
}
