//! `exa login`. Authenticates with Exa via API key, opening the dashboard
//! in the browser if interactive, verifying against the API, and storing the key in the OS keystore.
...
const API_KEYS_URL: &str = "https://dashboard.exa.ai/api-keys";
...
pub fn run(args: LoginArgs) -> Result<()> {
        ...
        && !force
    {
        return Err(Error::invalid(format!("An account named '{existing}' already exists."))
            .fix(format!("Use --force to replace its key: exa login {existing} --force")));
    }
...

    {
        ...
        config.accounts.insert(
            name.clone(),
            AccountConfig {
                identity: format!("key-{}", &key[key.len().saturating_sub(4)..]),
    ...
    output::write(&obj! {
        ...
        "nextStep" => format!("exa search \"test query\" --account {name}"),
    });
    Ok(())
}
...
fn prompt_login_key(name: &str, no_browser: bool) -> Result<String> {
    if !std::io::stdin().is_terminal() {
        return Err(Error::invalid("No API key given and no terminal to prompt on.")
            .fix(format!("pbpaste | exa login {name} --api-key-stdin")));
    }

    eprintln!("To log in, copy or create an API key from Exa:");
    ...
    loop {
        let key = rpassword::prompt_password(format!("Paste your Exa API key for '{name}': "))
            .map_err(|e| Error::other("Could not read the API key.").detail(e.to_string()))?;
        let key = key.trim().to_string();
        if !key.is_empty() {
            return Ok(key);
        }
        eprintln!("API key cannot be empty. Paste your key from {API_KEYS_URL}");
    }
}
