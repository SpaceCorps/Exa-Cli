//! `exa accounts add|list|test|remove` command implementations.
...
fn add(name: String, api_key: Option<String>, force: bool, no_verify: bool) -> Result<()> {
        ...
        && !force
    {
        return Err(Error::invalid(format!("An account named '{existing}' already exists.")).fix(format!(
            "Pick a different name, or replace its key: exa accounts add {existing} --api-key <key> --force"
        )));
    }
    ...
    output::write(&obj! {
        ...
        "nextStep" => format!("exa search \"latest news\" --account {name}"),
    });
    Ok(())
}
...
fn prompt_key(name: &str) -> Result<String> {
    if !std::io::stdin().is_terminal() {
        return Err(Error::invalid("No API key given and no terminal to prompt on.")
            .fix(format!("pbpaste | exa accounts add {name} --api-key-stdin")));
    }
    loop {
...
fn remove(requested: &str, yes: bool) -> Result<()> {
    ...
    let Some((name, _)) = config.find(requested) else {
        return Err(
            Error::new(ErrorCode::NoAccount, format!("No account named '{requested}'.")).fix("exa accounts list")
        );
    };
    let name = name.clone();
    ...
    if !yes {
        if !std::io::stdin().is_terminal() {
            ...
            .fix(format!("exa accounts remove {name} --yes")));
        }
    ...
    output::write(&obj! {
        "status" => "removed",
        "name" => name,
        "note" => "The API key was deleted locally.",
    });
    Ok(())
}
